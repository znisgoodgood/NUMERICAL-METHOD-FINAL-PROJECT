...
import yfinance as yf #for stock information    
from datetime import datetime #for read time information

def totalnum(days,everymo,stock):#for number,stockprice,dividends of stock
    count = 0
    currentprice = 0
    totaldividends = 0
    num = 0
    ...
    true_days = days[0:len(days)-1]#get the time without lately month
    print(true_days)
    
    print(dividends['Dividends'])
    for i in true_days:#calculate different parameter of stocks
        ...
        if dividends['Dividends'][count]>0:#calculate dividends if there is dividends given
            print('current num is ',num)
            totaldividends += num*dividends['Dividends'][count]#find total dividends
            print('cumulate to today',totaldividends)
        num += everymo/startday['Close'][0]#number of stocks

        count += 1
    print(num)
    return num,currentprice,totaldividends
def outputdate(start,day,year):#output the set of invest date
    date_list = []
...
def main(page: ft.Page):  
    ...
    def date_click(e):#for user to key there date information to interface
        ...
        page.update()
    num = []
    def stock_click(e):#for user to key there stock they want
        stock = stock_txt.value
        stock_list.append(stock)
        choose_txt.value = str(f'this list is : {stock_list}')
        page.update()
    def result_click(e):#for get best combination to chosen stocks and money 
        month = month_text.value
        year = year_text.value
        day = day_text.value
        date_list = outputdate(month,day,year)
